...
func (p *ProjectDao) FindProjectById(ctx context.Context, projectCode int64) (pr *data.Project, err error) {
	err = p.conn.Session(ctx).Where("id=?", projectCode).Find(&pr).Error
...
func (p *ProjectDao) DeleteProjectCollection(ctx context.Context, memId int64, projectCode int64) error {
...
func (p *ProjectDao) FindProjectByPIdAndMemId(ctx context.Context, projectCode int64, memberId int64) (*data.ProjectAndMember, error) {
	...
	err := raw.Scan(&pms).Error // 执行查询并将结果扫描到pms切片
	return pms, err
}
...
func (p *ProjectDao) FindCollectByPIdAndMemId(ctx context.Context, projectCode int64, memberId int64) (bool, error) {
	...
	raw := session.Raw(sql, memberId, projectCode)
	err := raw.Scan(&count).Error // 执行查询并将结果扫描到pms切片
	return count > 0, err
}
...
func (p ProjectDao) FindCollectByMemId(ctx context.Context, memberId int64, page int64, size int64) ([]*data.ProjectAndMember, int64, error) {
	var pms []*data.ProjectAndMember
	session := p.conn.Session(ctx)
	index := (page - 1) * size
	...
	raw.Scan(&pms) // 执行查询并将结果扫描到pms切片

	var total int64
	query := fmt.Sprintf("member_code=?")
	err := session.Model(&data.ProjectCollection{}).Where(query, memberId).Count(&total).Error // 总数查询
	return pms, total, err
}

// FindProjectByMemId 根据用户id分页查询用户参与的项目列表
