...
func (p *ProjectLogDao) FindLogByMemberCode(background context.Context, memberId int64, page int64, pageSize int64) (list []*data.ProjectLog, total int64, err error) {
	session := p.conn.Session(background)
	offset := (page - 1) * pageSize
	err = session.Model(&data.ProjectLog{}).Where("member_code=?", memberId).Limit(int(pageSize)).Offset(int(offset)).Order("create_time desc").Find(&list).Error
