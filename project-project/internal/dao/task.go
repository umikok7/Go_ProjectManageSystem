...
func (t TaskDao) FindTaskByAssignTo(ctx context.Context, memberId int64, done int, page int64, pageSize int64) (tList []*data.Task, total int64, err error) {
	session := t.conn.Session(ctx)
	offset := (page - 1) * pageSize
	err = session.Model(&data.Task{}).Where("assign_to=? and deleted=0 and done=?", memberId, done).Limit(int(pageSize)).Offset(int(offset)).Find(&tList).Error
...
func (t TaskDao) SaveTask(ctx context.Context, conn database.DbConn, ts *data.Task) error {
	t.conn = conn.(*gorms.GormConn) // 此处tx可能未初始化，可能未nil，使用传进来的conn断言后实现事务连接
